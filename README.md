...
A web app for tracking solar project procurement, installation progress,
maintenance scheduling, per-site contact info, and warehouse inventory —
shared across everyone who visits it. Runs as an Express app, deployable on
[Vercel](https://vercel.com), backed by a [Turso](https://turso.tech)
database (SQLite-compatible, free tier available).

Because the data lives in a real hosted database instead of the browser,
anyone who opens the deployed site sees the same projects and inventory —
unlike a local-storage-only version, where every browser/device has its own
separate copy.
...
- `seed.js` — sample project data loaded once, only if the database is empty
- `seed-inventory.js` — sample inventory data loaded once, only if empty
- `vercel.json` — routes all `/api/*` requests to the single serverless function
- `public/` — the frontend (React via CDN, no build step needed): two tabs,
